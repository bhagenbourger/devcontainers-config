...
##################################################
# Check for the presence of the mopidy-jugobox repository path argument
##################################################
if [ -z "$1" ] || [ ! -d "$1" ]; then
  echo "Usage: $0 <path-to-mopidy-jugobox-repository>"
  exit 1
fi
MOPIDY_REPO_PATH="$1"
DEVCONTAINER_PATH="$(pwd)/github.com/bhagenbourger/mopidy-jugobox/.devcontainer"
...
cp "$DEVCONTAINER_PATH/mopidy/config.json" "$MOPIDY_REPO_PATH/.devcontainer_config/mopidy/config.json"